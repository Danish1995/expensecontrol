
import "./components/Expenses/Expenses.css";
import Expenses from "./components/Expenses/Expenses";
import NewExpense from "./components/NewExpense/NewExpense";

import { useState } from "react";
// import is also importatnt to import from some file
const Dummy_expenses = [
  {
    id: 'e1',
    title: 'Toilet Paper',
    amount: '94.12',
    date: new Date(2020, 7, 14),
  },
  { id: 'e2', title: 'New TV', amount: '799.49', date: new Date(2021, 2, 12) },
  {
    id: 'e3',
    title: 'Car Insurance',
    amount: '294.67',
    date: new Date(2021, 2, 28),
  },
  {
    id: 'e4',
    title: 'New Desk (Wooden)',
    amount: '450',
    date: new Date(2021, 5, 12),
  },
  {
    id: 'e5',
    title: 'New Desk (Wooden)',
    amount: '450',
    date: new Date(2021, 5, 12),
  },
];
const  App=()=> { //here it is arrow fuction another way of using functions
  
  const [expenses, setExpenses]=useState(Dummy_expenses);
  const addExpenseDataHandler = (expenseDataValue) => {
    setExpenses((prevExpense) => {
      return [expenseDataValue,...prevExpense];
    });
  
  };
  const filterDataHandler = (expenseDataValue) => {
    setExpenses((prevExpense) => {
      return [expenseDataValue,...prevExpense];
    });
  };

  return (
    <div >
      <h2>Let's get started!</h2>
      <NewExpense onDataRecive={addExpenseDataHandler}/>
      <Expenses item={expenses}></Expenses>
      { /* this a custom functional component and this is a way to coment somethin in a div */ }
    </div>
  );
}

export default App;


/* shift +alt +a multilien comment */

